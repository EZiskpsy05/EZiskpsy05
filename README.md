## 👋 Hi, I’m EZiskpsy05

I build and contribute to **open-source, user-focused software**, with interests in  
**security-aware development, UX/UI, and research-oriented tooling**.

I value **clean design, privacy-respecting practices, and thoughtful engineering** over quantity or noise.

🔗 Portfolio & contact: https://howtra.rf.gd

---

## 🧠 Focus Areas

- 🔐 Cybersecurity fundamentals & tooling  
- 🎨 Human-centered UX/UI design  
- 🧪 Research-informed software  
- 🌍 Open-source collaboration  

---

## 🧰 Technical Stack

**Languages**
- TypeScript, JavaScript, Python, Java  

**Web & UI**
- React, Next.js
- HTML/CSS
- UX prototyping & visual design  

**Security & Systems**
- Applied cryptography concepts  
- Privacy-conscious development  
- OS & networking fundamentals  

**Workflow**
- Git / GitHub
- Cross-project collaboration
- Documentation & technical writing  



*(Additional work and private repositories are available upon request.)*
``
